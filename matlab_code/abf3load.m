function [d,si,h]=abf3load(fn,varargin)
% ** function [d,si,h]=abf3load(fn,varargin)
% version a, modified to put in things that abfload2 did, like episodess
% modified 7/8/2018 jrh to include stimulus waveform and holding
% information
%
...
%         [d,si,h]=abf3load('d:\data01.abf','info') 
% abf3load will not load any data but return detailed information (header
% parameters) on the file in output variable h. d and si will be empty.
% In all other cases abf3load will load data. Optional input parameters
% listed below (= all except the file name) must be specified as
% parameter/value pairs, e.g. as in 
%         d=abf3load('d:\data01.abf','start',100,'stop','e');
%
% >>> INPUT VARIABLES >>>
...
%                                 be read by abf3load 'ieee-be' is the 
%                                 alternative.
%
...
% leaving h.fSignalGain at 200 (and the data values produced by abf3load too
...
% abf3load; instead, data is always put out according to the order inherent
% to the abf file (to be retrieved in header parameter h). In the previous
% version of abf3load, specifying an order different from the inherent
% channel order could result in wrong scaling of the data (if the scaling
% differed between channels).
%
% File History:
%   2019-11-08 Now allows extra data points to be recorded but they are ignored
%   2020-04-04 Added verbose as an optional parameter
...
% h.sProtocolPath=char(h.sProtocolPath'); % edited by Jordan 7/23/2014
...
if strcmp(h.fFileSignature,'ABF2')
  ...
  h.lFileStartTime=h.uFileStartTimeMS*.001;
    ...
    % make long copies of these two parameters, as this is what is produced
    % when reading abf2 files
      h.lEpochInitDuration=h.nEpochInitDuration;
      h.lEpochDurationInc=h.nEpochDurationInc;
      h.lEpochPulsePeriod=[];
      h.lEpochPulseWidth=[];
...
if h.fFileVersionNumber>=2
  ...
  %  this is the section that needs to be debugged to work with 2.0.3 (10.5 clampex, works   
  % with new 1550 digitizer and it’s 8 output DAC channels. FIX HERE JRH
  % -----------------------------------------------------------------------
  ...
  % --- read in the epoch section & copy some values to header h
  for i=1:EpochSection.llNumEntries
      ...
      %EPDsec(i)=ReadSection(fid,EpochSection.uBlockIndex*BLOCKSIZE+EpochSection.uBytes*(i-1),EpochInfoPerDAC);
  ...
  h.lNumSamplesPerEpisode=ProtocolSec.lNumSamplesPerEpisode; % jrh
%   % dkt 
%   h.digitalHolding=ProtcolSec.nDigitalHolding;
%   %
%   
...
initOffset = round(h.lNumSamplesPerEpisode/h.nADCNumChannels /64);
epochDurations = h.lEpochInitDuration; % separate epoch durations by channel
epochStartPts = initOffset +cumsum(epochDurations); % epoch start points (sample number) for each episode
h.ep.epochStartPts = epochStartPts; % epoch start point (within each episode) for epochs (0,9)
h.ep.epochType = h.nEpochType; % 0 is step, 1 is ramp?
h.ep.epochInitLevel = h.fEpochInitLevel; % splits string into two columns one for each channel, rows indicate individual epochs
h.ep.epochLevelInc = h.fEpochLevelInc; % splits string into two columns one for each channel, rows indicate individual epochs
h.ep.epochDuration = h.lEpochInitDuration; % epoch durations
h.ep.epochDurationInc = h.lEpochDurationInc;
h.ep.epochPulsePeriod = h.lEpochPulsePeriod; % epoch pulse period
h.ep.epochPulseWidth = h.lEpochPulseWidth; % epoch pulse width
%% -------------------------------------------------------------------------
...
for i=1:DACSection.llNumEntries
  tmp=ReadSection(fid,DACSection.uBlockIndex*BLOCKSIZE+DACSection.uBytes*(i-1),PerDACInfo);
  if tmp.nWaveformEnable
      % used a waveform for this channel
      h.fDACHoldingLevel(i)=tmp.fDACHoldingLevel;
      h.nInterEpisodeLevel(i)=tmp.nInterEpisodeLevel;
  end
  % time of tag entry from start of experiment in s (corresponding expisode
  % number, if applicable, will be determined later)
  %h.tags(i).timeSinceRecStart=tmp.lTagTime*h.synchArrTimeBase/1e6;
...
switch h.nOperationMode
  case 1
    ...
    if h.fFileVersionNumber>=2.0
      errordlg('abf3load currently does not work with data acquired in event-driven variable-length mode and ABF version 2.0','ABF version issue');
    else
  ...
  case {2,4,5}
    ...
    if rem(h.dataPts,h.nADCNumChannels)>0 || rem(h.dataPtsPerChan,h.lActualEpisodes)>0
      fclose(fid);
      disp('Number of data points not OK! Extra data points ignored!');
    end
...
switch fileSig
 case 'ABF ' % ** note the blank
   ...
   headPar={
     ...
     'fDACHoldingLevel', 1394, 'float', tmp; % dkt 1/15/2012
     'nEpochType', 2308, 'int16', tmp; % dkt 1/15/2012
     'fEpochInitLevel', 2348, 'float', tmp; % dkt 1/15/2012
     'fEpochLevelInc', 2428, 'float', tmp;
     'nEpochInitDuration', 2508, 'int16', tmp; % jrh 3/16/2014
     'nEpochDurationInc', 2548, 'int16', tmp;
  
     'nInterEpisodeLevel', 2304, 'int16', tmp;

     'nTelegraphEnable',4512,'int16',tmp;
     'fTelegraphAdditGain',4576,'float',tmp;
     'sProtocolPath',4898,'*char', repmat(-1,1,256);
     };
 case 'ABF2'
   % ************************
   %     abf version >= 2.0
...
function PerDACInfo=define_PerDACInfo
PerDACInfo={
'nDACNum','int16',1;
