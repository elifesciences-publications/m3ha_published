...
%         [d,si,h]=abf2load('d:\data01.abf','info') 
% abf2load will not load any data but return detailed information (header
% parameters) on the file in output variable h. d and si will be empty.
% In all other cases abf2load will load data. Optional input parameters
% listed below (= all except the file name) must be specified as
% parameter/value pairs, e.g. as in 
%         d=abf2load('d:\data01.abf','start',100,'stop','e');
%
% >>> INPUT VARIABLES >>>
...
%                                 be read by abf2load 'ieee-be' is the 
%                                 alternative.
% << OUTPUT VARIABLES <<<
...
% leaving h.fSignalGain at 200 (and the data values produced by abf2load too
...
% abf2load; instead, data is always put out according to the order inherent
% to the abf file (to be retrieved in header parameter h). In the previous
% version of abf2load, specifying an order different from the inherent
...
switch h.nOperationMode
  case 1
    ...
    if h.fFileVersionNumber>=2.0
      errordlg('abf2load currently does not work with data acquired in event-driven variable-length mode and ABF version 2.0','ABF version issue');
    else
